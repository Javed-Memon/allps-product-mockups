      ...
      <div style={{ background:"#1A1A18", borderRadius:10, padding:"12px 18px",
        ...
        <div>
          <div style={{ fontSize:14, fontWeight:700, color:"white", marginBottom:3 }}>
            📱 Candidate Consent Screens
          </div>
          <div style={{ fontSize:12, color:"rgba(255,255,255,0.55)", lineHeight:1.5 }}>
            Sample screens for all consent types. Interactive — click toggles and buttons. Engineer spec notes included on each screen.
          </div>
        </div>
      ...
      <div style={{ display:"flex", flexWrap:"wrap", gap:8, marginBottom:24 }}>
        {SCREEN_DEFS.map(s => (
          <button key={s.id} onClick={() => setActive(s.id)}
            style={{ padding:"8px 16px", borderRadius:10, cursor:"pointer", textAlign:"left",
        ...
        <div>
          <div style={{ background:"white", border:"1px solid rgba(0,0,0,0.08)",
            ...
            <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:12 }}>
              <div>
                <div style={{ fontSize:16, fontWeight:700, color:"#1A1A18" }}>{sel.label} — {sel.title}</div>
            ...
            {active === "A" && <>
              <div style={{ fontSize:13, color:"#374151", lineHeight:1.7, marginBottom:10 }}>
                Shown once per application, before the form is submitted. The candidate is not being asked to give consent — the lawful basis is Legitimate Interest. This screen is a transparency notice that tells them what happens to their data and that AI matching is involved.
              </div>
              <div style={{ fontSize:12, color:"#888", borderTop:"1px solid rgba(0,0,0,0.06)", paddingTop:10 }}>
                <strong>Key design rules:</strong> No toggle. No "I agree" button. A single "continue to application" CTA. Candidate can close this and proceed — the screen is informational. Log that the screen was shown, not "consented to."
              </div>
            </>}
            {active === "B" && <>
              <div style={{ fontSize:13, color:"#374151", lineHeight:1.7, marginBottom:10 }}>
                Shown after interview scheduling, before recording begins. This is the explicit consent gate for AI screening. The candidate must actively toggle the switch on and click confirm. The "No thanks" option must offer an alternative — a human-led interview — with equal visual prominence.
              </div>
              <div style={{ fontSize:12, color:"#888", borderTop:"1px solid rgba(0,0,0,0.06)", paddingTop:10 }}>
                <strong>Key design rules:</strong> Toggle off by default. Both CTAs equal in size. Declining must not end the application — it must route to a human-led alternative. Recording is physically blocked until consent is logged.
              </div>
            </>}
            {active === "C" && <>
              <div style={{ fontSize:13, color:"#374151", lineHeight:1.7, marginBottom:10 }}>
                Separate from Screen B even if both interviews occur in the same process. Each consent type is independent — a candidate might agree to screening but decline skills assessment. The system must not infer consent for C from consent for B.
              </div>
              <div style={{ fontSize:12, color:"#888", borderTop:"1px solid rgba(0,0,0,0.06)", paddingTop:10 }}>
                <strong>Key design rules:</strong> Same pattern as Screen B. Do not pre-tick because Screen B was already granted. consent_type=SKILLS is a separate database record.
              </div>
            </>}
            {active === "D" && <>
              <div style={{ fontSize:13, color:"#374151", lineHeight:1.7, marginBottom:10 }}>
                Shown at the end of the active hiring process, after the outcome is communicated. This must never appear before the outcome is known — framing it as part of the application process would create implicit pressure to consent. It must be clearly optional with no impact on the application.
              </div>
              <div style={{ fontSize:12, color:"#888", borderTop:"1px solid rgba(0,0,0,0.06)", paddingTop:10 }}>
                <strong>Key design rules:</strong> Toggle off by default. "No thanks" deletes data on schedule — it is not a penalised choice. Set expiry_date and renewal_count=0 at grant time.
              </div>
            </>}
            {active === "DR" && <>
              <div style={{ fontSize:13, color:"#374151", lineHeight:1.7, marginBottom:10 }}>
                Delivered by email 30 days before the consent window expires. The candidate clicks a link in the email which opens this screen. The expiry date must be shown clearly. The "No thanks" option must be equal in prominence to renewal — and must trigger immediate deletion, not a deletion reminder.
              </div>
              <div style={{ fontSize:12, color:"#888", borderTop:"1px solid rgba(0,0,0,0.06)", paddingTop:10 }}>
                <strong>Key design rules:</strong> Show specific expiry date. Show specific data categories being renewed. Toggle off by default (candidate must actively renew). Increment renewal_count on grant.
              </div>
            </>}
            {active === "E" && <>
              <div style={{ fontSize:13, color:"#374151", lineHeight:1.7, marginBottom:10 }}>
                The EU AI Act Art. 26(7) disclosure gate. This is a separate screen from Consent B or C — it logs a different obligation. Consent B/C establishes the GDPR lawful basis. Screen E establishes that the candidate was informed about what the AI does and does not assess, as required by the AI Act. Two separate log entries, two separate regulatory purposes.
              </div>
              <div style={{ fontSize:12, color:"#888", borderTop:"1px solid rgba(0,0,0,0.06)", paddingTop:10 }}>
                <strong>Key design rules:</strong> Full-screen blocking modal. No auto-proceed on timer. Candidate must click. The "I have a question" path must not bypass the gate. Use the interactive preview to see the acknowledgement state.
              </div>
            </>}
            {active === "F" && <>
              <div style={{ fontSize:13, color:"#374151", lineHeight:1.7, marginBottom:10 }}>
                The Candidate Data Portal. Accessible from a magic link in any email from the platform. Consolidates all data rights in one place: consent withdrawal per type, Art. 21 objection for C1, data download, data correction, and deletion request. Each action is independent and does not affect the others.
              </div>
              <div style={{ fontSize:12, color:"#888", borderTop:"1px solid rgba(0,0,0,0.06)", paddingTop:10 }}>
                <strong>Key design rules:</strong> Consent withdrawal and Art. 21 objection in separate, visually distinct sections. Withdrawal uses "Withdraw" — objection uses "Object." Deletion shows a warning about active applications. Try the interactive flows — withdraw a consent type, file an objection, request deletion.
              </div>
            </>}
          </div>
          ...
          <div style={{ background:"white", border:"1px solid rgba(0,0,0,0.08)", borderRadius:10, padding:16 }}>
            <div style={{ fontSize:11, fontWeight:700, color:"#888", textTransform:"uppercase",
              letterSpacing:"0.06em", marginBottom:12 }}>What gets logged</div>
            <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
              {active === "A" && [
                ["Table","(no consent_records entry — transparency screen only)"],
                ["Event logged","screen_shown_at, lia_version, candidate_id (hashed)"],
                ["Consent type","N/A — Legitimate Interest basis"],
              ].map(([k,v],i) => <LogRow key={i} k={k} v={v} />)}
              {active === "B" && [
                ["Table","consent_records"],
                ["consent_type","SCREENING"],
                ["Fields","candidate_id (hashed), version, granted_at or withdrawn_at, method=IN_APP"],
                ["Gate","interview_session.recording_allowed requires SCREENING granted_at IS NOT NULL"],
              ].map(([k,v],i) => <LogRow key={i} k={k} v={v} />)}
              {active === "C" && [
                ["Table","consent_records"],
                ["consent_type","SKILLS"],
                ["Fields","candidate_id (hashed), version, granted_at or withdrawn_at, method=IN_APP"],
                ["Gate","skills_session.recording_allowed requires SKILLS granted_at IS NOT NULL"],
                ["Note","Separate row from Consent B — never inferred from B"],
              ].map(([k,v],i) => <LogRow key={i} k={k} v={v} />)}
              {active === "D" && [
                ["Table","consent_records"],
                ["consent_type","TALENT_POOL"],
                ["Fields","candidate_id (hashed), version, granted_at, expiry_date (T+6 months), renewal_count=0"],
                ["On decline","No record written — candidate data deleted on role closure schedule"],
              ].map(([k,v],i) => <LogRow key={i} k={k} v={v} />)}
              {active === "DR" && [
                ["Table","consent_records (UPDATE existing row)"],
                ["consent_type","TALENT_POOL"],
                ["Fields updated","granted_at (renewed), expiry_date (renewed_at + 6 months), renewal_count++"],
                ["capture_method","EMAIL_LINK"],
                ["On decline","Immediate soft-delete triggered"],
              ].map(([k,v],i) => <LogRow key={i} k={k} v={v} />)}
              {active === "E" && [
                ["Table","interview_disclosures (separate from consent_records)"],
                ["Fields","session_id, candidate_id (hashed), disclosure_version, acknowledged_at"],
                ["Gate","recording.start() blocked until interview_disclosures.acknowledged_at IS NOT NULL"],
                ["Regulatory purpose","EU AI Act Art. 26(7) — separate from GDPR consent log"],
              ].map(([k,v],i) => <LogRow key={i} k={k} v={v} />)}
              {active === "F" && [
                ["Withdrawal","consent_records: withdrawn_at = NOW() for selected type"],
                ["Objection","c1_processing_records: processing_active=false, stopped_at=NOW()"],
                ["DSR deletion","dsr_tickets: type=DELETION, deadline=NOW()+30d, assigned_to=DPO"],
                ["All actions","confirmation email sent within 24h"],
              ].map(([k,v],i) => <LogRow key={i} k={k} v={v} />)}
            </div>
          </div>
        </div>
