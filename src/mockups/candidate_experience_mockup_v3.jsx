...
// ══════════════════════════════════════════════════════════════════
// COMPLIANCE & CONSENT DASHBOARD
// R-02, R-03, R-04, R-16, R-17, R-27, R-28 | Art.4,12,13,14,26(7),86
// ══════════════════════════════════════════════════════════════════

// ─── SEED DATA ───────────────────────────────────────────────────
const CONSENT_CANDIDATES = [
  { id: "C001", name: "Sarah Müller",      role: "Senior Embedded Engineer",  company: "Bosch CH",    joined: "2026-01-08",
    c1: { status: "active",   ts: "2026-01-08 09:14", ver: "v2.1", method: "in-app" },
    ...
    dsrs: [] },
  { id: "C002", name: "David Keller",      role: "Firmware Engineer",          company: "u-blox",      joined: "2026-01-12",
    c1: { status: "active",   ts: "2026-01-12 08:55", ver: "v2.1", method: "in-app" },
  ...
  { id: "C003", name: "Priya Sharma",      role: "Embedded Systems Lead",      company: "Sensirion",   joined: "2026-02-03",
    c1: { status: "active",   ts: "2026-02-03 10:10", ver: "v2.1", method: "in-app" },
    ...
    dsrs: [] },
  { id: "C004", name: "Marco Rossi",       role: "Senior Embedded Engineer",  company: "Bosch CH",    joined: "2026-01-20",
    c1: { status: "active",   ts: "2026-01-20 13:22", ver: "v2.1", method: "in-app" },
  ...
  { id: "C005", name: "Anna Berger",       role: "Finance Analyst",            company: "Novartis",    joined: "2026-02-14",
    c1: { status: "active",   ts: "2026-02-14 08:30", ver: "v2.1", method: "in-app" },
    ...
    dsrs: [] },
  { id: "C006", name: "Luca Bianchi",      role: "Sales Executive",            company: "SAP CH",      joined: "2026-03-01",
    c1: { status: "active",   ts: "2026-03-01 09:45", ver: "v2.1", method: "in-app" },
  ...
  { id: "C007", name: "Isabelle Dubois",   role: "Operations Manager",         company: "Nestlé CH",   joined: "2026-01-30",
    c1: { status: "active",   ts: "2026-01-30 10:20", ver: "v2.1", method: "in-app" },
...
const ROLES_COMPLIANCE = [
  { id: "R01", title: "Senior Embedded Engineer", client: "Bosch Automotive CH",   posted: "2026-01-05", candidates: 47,
    aiDisclosure: true,  humanGate: true,  overrideLog: true,  explainAI: true,  candidates_active: 12 },
  { id: "R02", title: "Firmware Engineer",         client: "u-blox · Thalwil",     posted: "2026-01-12", candidates: 31,
    aiDisclosure: true,  humanGate: true,  overrideLog: false, explainAI: true,  candidates_active: 8 },
  { id: "R03", title: "Embedded Systems Lead",     client: "Sensirion · Stäfa",    posted: "2026-02-03", candidates: 22,
    aiDisclosure: false, humanGate: true,  overrideLog: true,  explainAI: false, candidates_active: 5 },
  { id: "R04", title: "Finance Analyst",           client: "Novartis · Basel",     posted: "2026-02-14", candidates: 58,
    aiDisclosure: true,  humanGate: false, overrideLog: false, explainAI: false, candidates_active: 14 },
  { id: "R05", title: "Sales Executive",           client: "SAP Switzerland",      posted: "2026-03-01", candidates: 39,
    aiDisclosure: true,  humanGate: true,  overrideLog: true,  explainAI: true,  candidates_active: 9 },
];

const STAFF_TRAINING = [
  { name: "Clara Meier",   role: "Talent Acquisition Lead", completed: "2026-01-15", version: "v1.2", dueNext: "2027-01-15", status: "current" },
  { name: "Tom Weber",     role: "Recruiter",               completed: "2026-01-20", version: "v1.2", dueNext: "2027-01-20", status: "current" },
  { name: "Nina Koch",     role: "HR Manager",              completed: "2025-11-10", version: "v1.0", dueNext: "2026-11-10", status: "current" },
  { name: "Felix Baumann", role: "Recruiter",               completed: null,         version: null,   dueNext: null,        status: "overdue" },
  { name: "Lea Zimmermann",role: "Hiring Manager",          completed: "2026-02-28", version: "v1.2", dueNext: "2027-02-28", status: "current" },
];

const AUDIT_LOG = [
  { ts: "2026-03-12 14:32", candidateId: "C001***", role: "R01", module: "AI Screening Interview v2.4.1", event: "Interview scored", humanAction: "Recruiter approved — Clara M.", outcome: "Progressed" },
  { ts: "2026-03-12 11:15", candidateId: "C004***", role: "R01", module: "AI Matching v3.1.0",           event: "Match ranked",    humanAction: "Recruiter reviewed — Tom W.",  outcome: "Shortlisted" },
  { ts: "2026-03-11 16:40", candidateId: "C007***", role: "R02", module: "AI Skills Interview v2.4.1",  event: "Interview scored", humanAction: "Recruiter approved — Clara M.", outcome: "Progressed" },
  { ts: "2026-03-11 09:20", candidateId: "C002***", role: "R02", module: "AI Sourcing v1.8.3",          event: "Candidate sourced",humanAction: "Override applied — Tom W.",     outcome: "Declined (override)" },
  { ts: "2026-03-10 15:05", candidateId: "C005***", role: "R04", module: "AI Matching v3.1.0",          event: "Match ranked",    humanAction: "Recruiter reviewed — Nina K.",  outcome: "Shortlisted" },
  { ts: "2026-03-10 10:30", candidateId: "C006***", role: "R05", module: "AI Screening Interview v2.4.1",event: "Interview scored",humanAction: "Recruiter approved — Clara M.", outcome: "Progressed" },
];
...
function ConsentBadge({ status }) {
  const map = {
    ...
    "n/a":     { label: "N/A",       bg: "#F3F4F6", color: "#6B7280" },
  };
...
function ObligationCell({ ok }) {
  return (
    <div style={{ display: "flex", justifyContent: "center" }}>
      {ok
        ? <span style={{ width: 22, height: 22, borderRadius: "50%", background: "#DCFCE7", color: "#166534", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800 }}>✓</span>
        : <span style={{ width: 22, height: 22, borderRadius: "50%", background: "#FEE2E2", color: "#991B1B", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 12, fontWeight: 800 }}>✕</span>
      }
    </div>
  );
}
...
function StatBox({ label, value, sub, color, bg, icon }) {
  return (
    <div style={{ background: bg, border: `1px solid ${color}33`, borderRadius: 10, padding: "12px 14px", flex: "1 1 120px" }}>
...
function ConsentTab() {
  ...
  const [activeSubTab, setActiveSubTab]           = useState("table"); // table | renewals | dsr

  const allActive    = CONSENT_CANDIDATES.filter(c => [c.c1,c.c2,c.c3,c.c4].every(x => ["active","n/a"].includes(x.status))).length;
  const expiring     = CONSENT_CANDIDATES.filter(c => [c.c1,c.c2,c.c3,c.c4].some(x => x.status === "expiring")).length;
  const withdrawn    = CONSENT_CANDIDATES.filter(c => [c.c1,c.c2,c.c3,c.c4].some(x => x.status === "withdrawn")).length;
  const expired      = CONSENT_CANDIDATES.filter(c => [c.c1,c.c2,c.c3,c.c4].some(x => x.status === "expired")).length;
  const openDSRs     = CONSENT_CANDIDATES.flatMap(c => c.dsrs).filter(d => d.status !== "fulfilled").length;

  const filtered = filterStatus === "all" ? CONSENT_CANDIDATES
    : CONSENT_CANDIDATES.filter(c => [c.c1,c.c2,c.c3,c.c4].some(x => x.status === filterStatus));

  const renewalQueue = CONSENT_CANDIDATES.filter(c => c.c4.status === "expiring" || c.c4.status === "expired");

  const allDSRs = CONSENT_CANDIDATES.flatMap(c =>
    c.dsrs.map(d => ({ ...d, candidateName: c.name, candidateId: c.id, role: c.role }))
  );

  const TH = ({ children, center }) => (
    <th style={{ padding: "8px 10px", fontSize: 11, fontWeight: 700, color: "#6B7280", background: "#F9FAFB", textAlign: center ? "center" : "left", whiteSpace: "nowrap", borderBottom: "1px solid #E5E7EB" }}>
      {children}
    </th>
  );
  const TD = ({ children, center }) => (
    <td style={{ padding: "8px 10px", fontSize: 12, color: "#374151", borderBottom: "1px solid #F3F4F6", textAlign: center ? "center" : "left", verticalAlign: "middle" }}>
      {children}
    </td>
  );
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
        <StatBox label="All Consents Valid"  value={allActive}  sub={`of ${CONSENT_CANDIDATES.length} candidates`} color="#166534" bg="#DCFCE7" icon="✅" />
        <StatBox label="Expiring ≤60 days"   value={expiring}   sub="Renewal required soon"   color="#854D0E" bg="#FEF9C3" icon="⏳" />
        <StatBox label="Consent Withdrawn"   value={withdrawn}  sub="At least 1 type withdrawn" color="#991B1B" bg="#FEE2E2" icon="⚠️" />
        <StatBox label="Expired — Action Req" value={expired}   sub="Talent pool consent lapsed" color="#92400E" bg="#FEF3C7" icon="🔴" />
        <StatBox label="Open DSRs"           value={openDSRs}   sub="Within 30-day SLA"       color="#1E40AF" bg="#EFF6FF" icon="📋" />
      </div>

      {/* Sub-tab nav */}
      <div style={{ display: "flex", gap: 0, marginBottom: 14, background: "#F3F4F6", borderRadius: 8, padding: 4 }}>
        {[
          { id: "table",    label: "Candidate Consent Table" },
          { id: "renewals", label: `Renewal Queue (${renewalQueue.length})` },
          { id: "dsr",      label: `DSR Tracker (${allDSRs.length})` },
        ].map(t => (
          <button key={t.id} onClick={() => setActiveSubTab(t.id)} style={{
            flex: 1, padding: "7px 10px", fontSize: 12, fontWeight: 700, border: "none", cursor: "pointer", borderRadius: 6,
            background: activeSubTab === t.id ? "#FFFFFF" : "transparent",
            color: activeSubTab === t.id ? "#0F2A4A" : "#6B7280",
      ...
      {activeSubTab === "table" && (
        <div>
          ...
          <div style={{ display: "flex", gap: 6, marginBottom: 10, flexWrap: "wrap" }}>
            {["all","active","expiring","expired","withdrawn","pending"].map(f => (
              <button key={f} onClick={() => setFilterStatus(f)} style={{
                ...
                background: filterStatus === f ? "#0F2A4A" : "#FFFFFF",
                color: filterStatus === f ? "#FFFFFF" : "#6B7280",
                border: `1.5px solid ${filterStatus === f ? "#0F2A4A" : "#E5E7EB"}`,
              ...
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 760 }}>
                <thead>
                  <tr>
                    <TH>Candidate</TH>
                    <TH>Role / Client</TH>
                    <TH center>C1 — Application & Matching (LI)</TH>
                    <TH center>C2 — AI Screening (Consent)</TH>
                    <TH center>C3 — AI Skills Interview (Consent)</TH>
                    <TH center>C4 — Talent Pool (Consent)</TH>
                    <TH center>DSRs</TH>
                    <TH center>Detail</TH>
                  </tr>
                </thead>
                <tbody>
                  {filtered.map(c => (
                    <tr key={c.id} style={{ background: selectedCandidate?.id === c.id ? "#EFF6FF" : "#FFFFFF" }}>
                      <TD>
                      ...
                      <TD>
                        <div>{c.role}</div>
                        <div style={{ fontSize: 10, color: "#9CA3AF" }}>{c.company}</div>
                      </TD>
                      <TD center><ConsentBadge status={c.c1.status} /></TD>
                      <TD center><ConsentBadge status={c.c2.status} /></TD>
                      <TD center><ConsentBadge status={c.c3.status} /></TD>
                      <TD center><ConsentBadge status={c.c4.status} /></TD>
                      <TD center>
                        {c.dsrs.length > 0
                          ? <span style={{ fontSize: 11, fontWeight: 700, color: "#1E40AF", background: "#EFF6FF", padding: "2px 7px", borderRadius: 10 }}>{c.dsrs.length}</span>
                          : <span style={{ fontSize: 11, color: "#9CA3AF" }}>—</span>
                        }
                      </TD>
                      <TD center>
                        <button onClick={() => setSelectedCandidate(selectedCandidate?.id === c.id ? null : c)}
                          style={{ fontSize: 11, fontWeight: 600, color: "#1B5FA8", background: "#EFF6FF", border: "none", padding: "4px 10px", borderRadius: 6, cursor: "pointer" }}>
                          {selectedCandidate?.id === c.id ? "Close" : "View"}
                        </button>
                      </TD>
                    </tr>
                  ))}
          ...
          {selectedCandidate && (
            <div style={{ marginTop: 12, background: "#FFFFFF", border: "2px solid #1B5FA8", borderRadius: 10, padding: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
                <div>
                  <span style={{ fontSize: 14, fontWeight: 800, color: "#0F2A4A" }}>{selectedCandidate.name}</span>
                  <span style={{ fontSize: 11, color: "#9CA3AF", marginLeft: 8 }}>Consent Record — {selectedCandidate.id}</span>
                </div>
                <button onClick={() => setSelectedCandidate(null)} style={{ background: "none", border: "none", fontSize: 18, color: "#9CA3AF", cursor: "pointer" }}>✕</button>
              </div>
              <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(200px, 1fr))", gap: 10 }}>
                {[
                  { key: "c1", label: "C1 — Application & AI Matching", basis: "Legitimate Interest — Art. 6(1)(f)" },
                  ...
                  const rec = selectedCandidate[key];
                  return (
                    <div key={key} style={{ background: "#F9FAFB", borderRadius: 8, padding: "10px 12px", border: "1px solid #E5E7EB" }}>
                      <div style={{ fontSize: 11, fontWeight: 700, color: "#374151", marginBottom: 4 }}>{label}</div>
                      <div style={{ marginBottom: 6 }}><ConsentBadge status={rec.status} /></div>
                      <div style={{ fontSize: 10, color: "#6B7280", lineHeight: 1.6 }}>
                        <div><b>Basis:</b> {basis}</div>
                        {rec.ts && <div><b>Captured:</b> {rec.ts}</div>}
                        {rec.ver && <div><b>Version:</b> {rec.ver}</div>}
                        {rec.method && <div><b>Method:</b> {rec.method}</div>}
                        {rec.withdrawnAt && <div style={{ color: "#991B1B" }}><b>Withdrawn:</b> {rec.withdrawnAt}</div>}
                        {rec.expiry && <div><b>Expiry:</b> {rec.expiry}</div>}
                        {rec.renewals !== undefined && <div><b>Renewals:</b> {rec.renewals}</div>}
                        {rec.status === "n/a" && <div style={{ fontStyle: "italic" }}>Not applicable for this process</div>}
                      </div>
                    </div>
                  );
                })}
              </div>
              {selectedCandidate.dsrs.length > 0 && (
                <div style={{ marginTop: 12, background: "#EFF6FF", borderRadius: 8, padding: "10px 12px" }}>
                  <div style={{ fontSize: 11, fontWeight: 700, color: "#1E40AF", marginBottom: 6 }}>Data Subject Requests</div>
                  {selectedCandidate.dsrs.map((d, i) => (
                    <div key={i} style={{ fontSize: 11, color: "#374151", display: "flex", gap: 10, alignItems: "center" }}>
      ...
      {activeSubTab === "renewals" && (
        <div>
          <div style={{ background: "#FEF9C3", border: "1px solid #FDE68A", borderLeft: "4px solid #B45309", borderRadius: 8, padding: "10px 14px", marginBottom: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#854D0E", marginBottom: 4 }}>R-04 · GDPR Art. 7 · nFADP Art. 6</div>
            <div style={{ fontSize: 12, color: "#713F12", lineHeight: 1.6 }}>
              Talent Pool consent must be renewed every 6 months. Renewal reminders must reference the specific data scope being renewed — not use generic language. Candidates not renewing must be auto-deleted at T+12 months.
            </div>
          </div>
          <div style={{ background: "#FFFFFF", borderRadius: 10, border: "1px solid #E5E7EB", overflow: "hidden" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  {["Candidate","Role / Client","C4 Status","Consent Captured","Expiry Date","Renewals","Action"].map((h, i) => (
                    <TH key={h} center={i >= 2}>{h}</TH>
                  ))}
                </tr>
              </thead>
              <tbody>
                {renewalQueue.map(c => {
                  const daysLeft = c.c4.expiry
                    ? Math.ceil((new Date(c.c4.expiry) - new Date("2026-05-12")) / 86400000)
                    : null;
                  return (
                    <tr key={c.id}>
                      <TD><div style={{ fontWeight: 700, color: "#111827" }}>{c.name}</div><div style={{ fontSize: 10, color: "#9CA3AF" }}>{c.id}</div></TD>
                      <TD><div>{c.role}</div><div style={{ fontSize: 10, color: "#9CA3AF" }}>{c.company}</div></TD>
                      <TD center><ConsentBadge status={c.c4.status} /></TD>
                      <TD center><span style={{ fontSize: 11, color: "#6B7280" }}>{c.c4.ts?.split(" ")[0] || "—"}</span></TD>
                      <TD center>
                        <span style={{ fontSize: 11, fontWeight: 600, color: daysLeft !== null && daysLeft < 30 ? "#991B1B" : "#6B7280" }}>
                          {c.c4.expiry || "Expired"}
                          {daysLeft !== null && <div style={{ fontSize: 9 }}>{daysLeft > 0 ? `${daysLeft}d left` : "OVERDUE"}</div>}
                        </span>
                      </TD>
                      <TD center><span style={{ fontSize: 12, fontWeight: 700, color: "#374151" }}>{c.c4.renewals ?? "—"}</span></TD>
                      <TD center>
                        <button style={{ fontSize: 11, fontWeight: 600, color: "#FFFFFF", background: "#B45309", border: "none", padding: "5px 12px", borderRadius: 6, cursor: "pointer" }}>
                          Send Renewal
                        </button>
                      </TD>
                    </tr>
          ...
          <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
            <button style={{ fontSize: 12, fontWeight: 600, color: "#FFFFFF", background: "#0F2A4A", border: "none", padding: "8px 18px", borderRadius: 8, cursor: "pointer" }}>
              Send All Renewal Reminders ({renewalQueue.length})
            </button>
            <button style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", background: "#F3F4F6", border: "1px solid #E5E7EB", padding: "8px 18px", borderRadius: 8, cursor: "pointer" }}>
      ...
      {activeSubTab === "dsr" && (
        <div>
          <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderLeft: "4px solid #1B5FA8", borderRadius: 8, padding: "10px 14px", marginBottom: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#1E40AF", marginBottom: 4 }}>R-27 · R-28 · GDPR Art. 12, 15–22 — 30-day fulfilment SLA</div>
            <div style={{ fontSize: 12, color: "#1E3A5F", lineHeight: 1.6 }}>
              All Data Subject Requests must be acknowledged within 3 business days and fulfilled within 30 calendar days. DSR types: Access · Deletion · Correction · Portability · Objection.
            </div>
          </div>
          <div style={{ background: "#FFFFFF", borderRadius: 10, border: "1px solid #E5E7EB", overflow: "hidden" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  {["Candidate","Type","Filed","Due (30d SLA)","Status","Assignee","Action"].map((h, i) => (
                    <TH key={h} center={i >= 3}>{h}</TH>
                  ))}
                </tr>
              </thead>
              <tbody>
                {allDSRs.map((d, i) => {
                  ...
                  const overdue  = daysLeft < 0 && d.status !== "fulfilled";
                  return (
                    <tr key={i} style={{ background: overdue ? "#FFF1F2" : "#FFFFFF" }}>
                      <TD>
                        <div style={{ fontWeight: 700 }}>{d.candidateName}</div>
                        <div style={{ fontSize: 10, color: "#9CA3AF" }}>{d.candidateId} · {d.role}</div>
                      </TD>
                      <TD>
                        <span style={{ fontSize: 11, fontWeight: 600, background: "#F3F4F6", padding: "2px 8px", borderRadius: 4 }}>{d.type}</span>
                      </TD>
                      <TD><span style={{ fontSize: 11, color: "#6B7280" }}>{d.filed}</span></TD>
                      <TD center>
                        <span style={{ fontSize: 11, fontWeight: 600, color: overdue ? "#991B1B" : daysLeft <= 5 ? "#B45309" : "#374151" }}>
                          {d.due}
                          {d.status !== "fulfilled" && <div style={{ fontSize: 9 }}>{overdue ? "OVERDUE" : `${daysLeft}d left`}</div>}
                        </span>
                      </TD>
                      <TD center><DSRBadge status={d.status} /></TD>
                      <TD center><span style={{ fontSize: 11, color: "#6B7280" }}>{d.assignee}</span></TD>
                      <TD center>
                        {d.status !== "fulfilled"
                          ? <button style={{ fontSize: 11, fontWeight: 600, color: "#1B5FA8", background: "#EFF6FF", border: "none", padding: "4px 10px", borderRadius: 6, cursor: "pointer" }}>Open</button>
...
function AIActTab() {
  ...
  const [activeSubTab, setActiveSubTab] = useState("scorecard"); // scorecard | roles | training | audit

  const compliantRoles    = ROLES_COMPLIANCE.filter(r => r.aiDisclosure && r.humanGate && r.overrideLog && r.explainAI).length;
  const partialRoles      = ROLES_COMPLIANCE.filter(r => {
    const ok = [r.aiDisclosure, r.humanGate, r.overrideLog, r.explainAI].filter(Boolean).length;
    return ok >= 2 && ok < 4;
  }).length;
  const nonCompliantRoles = ROLES_COMPLIANCE.length - compliantRoles - partialRoles;
  const overdueTraining   = STAFF_TRAINING.filter(s => s.status === "overdue").length;
  const daysToDeadline    = Math.ceil((new Date("2026-08-02") - new Date("2026-05-12")) / 86400000);

  const OBLIGATIONS = [
    { key: "aiDisclosure", label: "AI Disclosure on Job Posting", article: "Art. 26(7)", desc: "Mandatory disclosure that AI is used in hiring, on every live job posting." },
    { key: "humanGate",    label: "Human-in-the-Loop Gate",       article: "Art. 14",    desc: "Recruiter must explicitly approve/reject/hold before decision reaches candidate." },
    { key: "overrideLog",  label: "Override Logging Active",      article: "Art. 14",    desc: "Recruiter overrides of AI recommendations are logged with documented reasons." },
    { key: "explainAI",    label: "Explain My Assessment Enabled",article: "Art. 86",    desc: "Candidate-facing explanation of AI assessment factors is available for this role." },
  ];

  const TH = ({ children, center }) => (
    <th style={{ padding: "8px 10px", fontSize: 11, fontWeight: 700, color: "#6B7280", background: "#F9FAFB", textAlign: center ? "center" : "left", whiteSpace: "nowrap", borderBottom: "1px solid #E5E7EB" }}>{children}</th>
  );
  const TD = ({ children, center }) => (
    <td style={{ padding: "8px 10px", fontSize: 12, color: "#374151", borderBottom: "1px solid #F3F4F6", textAlign: center ? "center" : "left", verticalAlign: "middle" }}>{children}</td>
  );

  return (
    <div>
      {/* Deadline banner */}
      <div style={{ background: daysToDeadline < 120 ? "#FEE2E2" : "#FEF9C3", border: `1px solid ${daysToDeadline < 120 ? "#FECACA" : "#FDE68A"}`, borderLeft: `4px solid ${daysToDeadline < 120 ? "#991B1B" : "#B45309"}`, borderRadius: 8, padding: "10px 14px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontSize: 11, fontWeight: 700, color: daysToDeadline < 120 ? "#991B1B" : "#854D0E" }}>⚠ EU AI Act High-Risk Deadline — 2 August 2026</div>
          <div style={{ fontSize: 12, color: "#374151", marginTop: 2 }}>All Arts. 9–15, 26, 49, 86 obligations must be fulfilled for all active roles. Arts. 4 & 26(7) are already in force.</div>
        </div>
        <div style={{ textAlign: "right", flexShrink: 0 }}>
          <div style={{ fontSize: 26, fontWeight: 800, color: daysToDeadline < 120 ? "#991B1B" : "#854D0E", lineHeight: 1 }}>{daysToDeadline}</div>
      ...
      {/* Stats */}
      <div style={{ display: "flex", gap: 10, flexWrap: "wrap", marginBottom: 16 }}>
        <StatBox label="Fully Compliant Roles"    value={compliantRoles}    sub="All 4 obligations met"    color="#166534" bg="#DCFCE7" icon="✅" />
        <StatBox label="Partially Compliant"      value={partialRoles}      sub="1–3 obligations missing"  color="#854D0E" bg="#FEF9C3" icon="⚠️" />
        <StatBox label="Non-Compliant Roles"      value={nonCompliantRoles} sub="Immediate action required" color="#991B1B" bg="#FEE2E2" icon="🔴" />
        <StatBox label="AI Literacy Overdue"      value={overdueTraining}   sub="Art. 4 — already in force" color="#1E40AF" bg="#EFF6FF" icon="🎓" />
      </div>

      {/* Sub-tab nav */}
      <div style={{ display: "flex", gap: 0, marginBottom: 14, background: "#F3F4F6", borderRadius: 8, padding: 4 }}>
        {[
          { id: "scorecard", label: "Deployer Scorecard" },
          { id: "roles",     label: "Role-Level Checklist" },
          { id: "training",  label: "AI Literacy Training" },
      ...
      {activeSubTab === "scorecard" && (
        <div style={{ display: "flex", gap: 14, flexWrap: "wrap" }}>
          <div style={{ flex: "1 1 300px" }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#374151", marginBottom: 10 }}>Overall Compliance Score — Active Roles</div>
            <div style={{ background: "#FFFFFF", borderRadius: 10, border: "1px solid #E5E7EB", overflow: "hidden" }}>
              {OBLIGATIONS.map(ob => {
                const met = ROLES_COMPLIANCE.filter(r => r[ob.key]).length;
                const pct = Math.round((met / ROLES_COMPLIANCE.length) * 100);
                return (
                  <div key={ob.key} style={{ padding: "12px 14px", borderBottom: "1px solid #F3F4F6" }}>
                    <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                      <div>
                        <span style={{ fontSize: 12, fontWeight: 700, color: "#111827" }}>{ob.label}</span>
                        <span style={{ fontSize: 10, color: "#9CA3AF", marginLeft: 6 }}>{ob.article}</span>
                      </div>
                      <span style={{ fontSize: 12, fontWeight: 800, color: pct === 100 ? "#166534" : pct >= 60 ? "#854D0E" : "#991B1B" }}>{pct}%</span>
                    </div>
                    <div style={{ height: 6, background: "#F3F4F6", borderRadius: 99 }}>
                      <div style={{ height: "100%", width: `${pct}%`, background: pct === 100 ? "#22C55E" : pct >= 60 ? "#F59E0B" : "#EF4444", borderRadius: 99, transition: "width 0.5s" }} />
                    </div>
                    <div style={{ fontSize: 10, color: "#6B7280", marginTop: 4 }}>{ob.desc}</div>
                  </div>
                );
              })}
            </div>
          ...
          <div style={{ flex: "1 1 240px" }}>
            <div style={{ fontSize: 12, fontWeight: 700, color: "#374151", marginBottom: 10 }}>Obligations Reference</div>
            {[
              { art: "Art. 4",     status: "PAST",    label: "AI Literacy Training",         color: "#991B1B" },
              { art: "Art. 26(7)", status: "PAST",    label: "Candidate AI Disclosure",       color: "#991B1B" },
              { art: "Art. 13",    status: "AUG 2026",label: "Instructions for Use to deployers", color: "#854D0E" },
              { art: "Art. 14",    status: "AUG 2026",label: "Human Oversight Controls",      color: "#854D0E" },
              { art: "Art. 12",    status: "AUG 2026",label: "Immutable Audit Logs (6mo)",    color: "#854D0E" },
              { art: "Art. 86",    status: "AUG 2026",label: "Right to Explanation",          color: "#854D0E" },
              { art: "Art. 49",    status: "AUG 2026",label: "EU AI Database Registration",   color: "#854D0E" },
            ].map(item => (
              <div key={item.art} style={{ display: "flex", gap: 10, alignItems: "center", padding: "7px 10px", background: "#FFFFFF", borderRadius: 8, marginBottom: 5, border: "1px solid #E5E7EB" }}>
                <span style={{ fontSize: 10, fontWeight: 700, color: "#FFFFFF", background: item.color, padding: "2px 6px", borderRadius: 4, whiteSpace: "nowrap" }}>{item.art}</span>
                <div style={{ flex: 1 }}>
                  <div style={{ fontSize: 11, fontWeight: 600, color: "#374151" }}>{item.label}</div>
                  <div style={{ fontSize: 9, color: item.color, fontWeight: 700 }}>{item.status === "PAST" ? "⚠ ALREADY IN FORCE" : `Deadline: ${item.status}`}</div>
                </div>
              </div>
            ))}
          </div>
            ...
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
                <thead>
                  <tr>
                    <TH>Role / Client</TH>
                    <TH center>AI Disclosure Art.26(7)</TH>
                    <TH center>Human Gate Art.14</TH>
                    <TH center>Override Log Art.14</TH>
                    <TH center>Explain AI Art.86</TH>
                    <TH center>Overall</TH>
                    <TH center>Detail</TH>
                  </tr>
                </thead>
                <tbody>
                  {ROLES_COMPLIANCE.map(role => {
                    const score = [role.aiDisclosure, role.humanGate, role.overrideLog, role.explainAI].filter(Boolean).length;
                    const overall = score === 4 ? "compliant" : score >= 2 ? "partial" : "non-compliant";
                    return (
                      <tr key={role.id} style={{ background: selectedRole?.id === role.id ? "#EFF6FF" : "#FFFFFF" }}>
                        <TD>
                          <div style={{ fontWeight: 700, color: "#111827" }}>{role.title}</div>
                          <div style={{ fontSize: 10, color: "#9CA3AF" }}>{role.client} · Posted {role.posted}</div>
                        </TD>
                        <TD center><ObligationCell ok={role.aiDisclosure} /></TD>
                        <TD center><ObligationCell ok={role.humanGate} /></TD>
                        <TD center><ObligationCell ok={role.overrideLog} /></TD>
                        <TD center><ObligationCell ok={role.explainAI} /></TD>
                        <TD center>
                          <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 10,
                            background: overall === "compliant" ? "#DCFCE7" : overall === "partial" ? "#FEF9C3" : "#FEE2E2",
                            color: overall === "compliant" ? "#166534" : overall === "partial" ? "#854D0E" : "#991B1B",
                          }}>{overall === "compliant" ? "✓ Compliant" : overall === "partial" ? "⚠ Partial" : "✕ Non-compliant"}</span>
                        </TD>
                        <TD center>
                          <button onClick={() => setSelectedRole(selectedRole?.id === role.id ? null : role)}
                            style={{ fontSize: 11, fontWeight: 600, color: "#1B5FA8", background: "#EFF6FF", border: "none", padding: "4px 10px", borderRadius: 6, cursor: "pointer" }}>
                            {selectedRole?.id === role.id ? "Close" : "Fix"}
                          </button>
                        </TD>
                      </tr>
                    );
          ...
          {selectedRole && (
            <div style={{ marginTop: 12, background: "#FFFFFF", border: "2px solid #1B5FA8", borderRadius: 10, padding: 16 }}>
              <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 10 }}>
                <span style={{ fontSize: 14, fontWeight: 800, color: "#0F2A4A" }}>Remediation: {selectedRole.title}</span>
                <button onClick={() => setSelectedRole(null)} style={{ background: "none", border: "none", fontSize: 18, color: "#9CA3AF", cursor: "pointer" }}>✕</button>
              </div>
              {OBLIGATIONS.filter(ob => !selectedRole[ob.key]).map(ob => (
                <div key={ob.key} style={{ background: "#FEF2F2", border: "1px solid #FECACA", borderRadius: 8, padding: "10px 12px", marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", marginBottom: 4 }}>
                    <span style={{ fontSize: 12, fontWeight: 700, color: "#991B1B" }}>✕ Missing: {ob.label}</span>
                    <span style={{ fontSize: 10, fontWeight: 700, color: "#FFFFFF", background: "#991B1B", padding: "1px 6px", borderRadius: 4 }}>{ob.article}</span>
                  </div>
                  <div style={{ fontSize: 11, color: "#7F1D1D", marginBottom: 8 }}>{ob.desc}</div>
                  <button style={{ fontSize: 11, fontWeight: 600, color: "#FFFFFF", background: "#1B5FA8", border: "none", padding: "5px 14px", borderRadius: 6, cursor: "pointer" }}>
                    Enable Now →
                  </button>
                </div>
              ))}
              {OBLIGATIONS.filter(ob => selectedRole[ob.key]).map(ob => (
                <div key={ob.key} style={{ background: "#F0FDF4", border: "1px solid #BBF7D0", borderRadius: 8, padding: "8px 12px", marginBottom: 6, display: "flex", gap: 8, alignItems: "center" }}>
                  <span style={{ color: "#166534", fontSize: 14 }}>✓</span>
                  <span style={{ fontSize: 12, color: "#166534", fontWeight: 600 }}>{ob.label}</span>
                  <span style={{ fontSize: 10, color: "#9CA3AF", marginLeft: "auto" }}>{ob.article}</span>
                </div>
              ))}
            </div>
          )}
      ...
      {activeSubTab === "training" && (
        <div>
          <div style={{ background: "#FEE2E2", border: "1px solid #FECACA", borderLeft: "4px solid #991B1B", borderRadius: 8, padding: "10px 14px", marginBottom: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#991B1B", marginBottom: 4 }}>Art. 4 — AI Literacy — Deadline Passed: 2 February 2025</div>
            <div style={{ fontSize: 12, color: "#7F1D1D", lineHeight: 1.6 }}>All staff who interact with AI outputs must have completed AI literacy training. Overdue staff must be enrolled immediately. Completions must be logged with version and date.</div>
          </div>
          <div style={{ background: "#FFFFFF", borderRadius: 10, border: "1px solid #E5E7EB", overflow: "hidden" }}>
            <table style={{ width: "100%", borderCollapse: "collapse" }}>
              <thead>
                <tr>
                  {["Staff Member","Role","Status","Completed","Version","Next Due","Action"].map((h, i) => (
                    <TH key={h} center={i >= 2}>{h}</TH>
                  ))}
                </tr>
              </thead>
              <tbody>
                {STAFF_TRAINING.map((s, i) => (
                  <tr key={i} style={{ background: s.status === "overdue" ? "#FFF1F2" : "#FFFFFF" }}>
                    ...
                    <TD center>
                      <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 8px", borderRadius: 10,
                        background: s.status === "current" ? "#DCFCE7" : "#FEE2E2",
                        color: s.status === "current" ? "#166534" : "#991B1B",
                      }}>{s.status === "current" ? "✓ Current" : "⚠ OVERDUE"}</span>
                    </TD>
                    <TD center><span style={{ fontSize: 11, color: "#6B7280" }}>{s.completed || "—"}</span></TD>
                    <TD center><span style={{ fontSize: 11, color: "#6B7280" }}>{s.version || "—"}</span></TD>
                    <TD center><span style={{ fontSize: 11, color: s.status === "overdue" ? "#991B1B" : "#6B7280", fontWeight: s.status === "overdue" ? 700 : 400 }}>{s.dueNext || "IMMEDIATE"}</span></TD>
                    <TD center>
                      {s.status === "overdue"
                        ? <button style={{ fontSize: 11, fontWeight: 700, color: "#FFFFFF", background: "#991B1B", border: "none", padding: "4px 10px", borderRadius: 6, cursor: "pointer" }}>Enrol Now</button>
                        : <button style={{ fontSize: 11, fontWeight: 600, color: "#6B7280", background: "#F3F4F6", border: "none", padding: "4px 10px", borderRadius: 6, cursor: "pointer" }}>View Record</button>
                      }
                    </TD>
                  </tr>
                ))}
      ...
      {activeSubTab === "audit" && (
        <div>
          <div style={{ background: "#EFF6FF", border: "1px solid #BFDBFE", borderLeft: "4px solid #1B5FA8", borderRadius: 8, padding: "10px 14px", marginBottom: 14 }}>
            <div style={{ fontSize: 11, fontWeight: 700, color: "#1E40AF", marginBottom: 4 }}>Art. 12 — Immutable Audit Log — Minimum 6-month retention</div>
            <div style={{ fontSize: 12, color: "#1E3A5F", lineHeight: 1.6 }}>Every AI decision event must be logged with: hashed candidate ID, job ID, AI module name and version, input features used, output decision, timestamp, and the human reviewer's action. Logs are append-only and cannot be modified.</div>
          </div>
          <div style={{ background: "#FFFFFF", borderRadius: 10, border: "1px solid #E5E7EB", overflow: "hidden" }}>
            <div style={{ overflowX: "auto" }}>
              <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
                <thead>
                  <tr>
                    {["Timestamp","Candidate (Hashed)","Role","AI Module & Version","Event","Human Action","Outcome"].map(h => (
                      <TH key={h}>{h}</TH>
                    ))}
                  </tr>
                </thead>
                <tbody>
                  {AUDIT_LOG.map((entry, i) => (
                    <tr key={i}>
                      <TD><span style={{ fontSize: 11, color: "#6B7280", whiteSpace: "nowrap" }}>{entry.ts}</span></TD>
                      <TD><span style={{ fontSize: 11, fontFamily: "monospace", background: "#F3F4F6", padding: "1px 6px", borderRadius: 4 }}>{entry.candidateId}</span></TD>
                      <TD><span style={{ fontSize: 11, color: "#6B7280" }}>{entry.role}</span></TD>
                      <TD><span style={{ fontSize: 11, color: "#374151" }}>{entry.module}</span></TD>
                      <TD><span style={{ fontSize: 11, background: "#EFF6FF", color: "#1E40AF", padding: "1px 6px", borderRadius: 4 }}>{entry.event}</span></TD>
                      <TD><span style={{ fontSize: 11, color: "#374151" }}>{entry.humanAction}</span></TD>
                      <TD>
                        <span style={{ fontSize: 10, fontWeight: 700, padding: "2px 7px", borderRadius: 10,
                          background: entry.outcome.includes("override") ? "#FEF3C7" : entry.outcome === "Progressed" ? "#DCFCE7" : "#EFF6FF",
                          color: entry.outcome.includes("override") ? "#92400E" : entry.outcome === "Progressed" ? "#166534" : "#1E40AF",
                        }}>{entry.outcome}</span>
                      </TD>
                    </tr>
                  ))}
          ...
          <div style={{ marginTop: 10, display: "flex", gap: 8 }}>
            <button style={{ fontSize: 12, fontWeight: 600, color: "#FFFFFF", background: "#0F2A4A", border: "none", padding: "8px 18px", borderRadius: 8, cursor: "pointer" }}>Export Log (JSON)</button>
            <button style={{ fontSize: 12, fontWeight: 600, color: "#6B7280", background: "#F3F4F6", border: "1px solid #E5E7EB", padding: "8px 18px", borderRadius: 8, cursor: "pointer" }}>Export Log (CSV)</button>
          </div>
        </div>
      )}
    </div>
...
// ─── COMPLIANCE SCREEN (top-level, both tabs) ─────────────────────
function ComplianceScreen() {
  ...
  return (
    <div>
      {/* Header */}
      <div style={{ background: "#0F2A4A", borderRadius: 10, padding: "14px 18px", marginBottom: 16, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
        <div>
          <div style={{ fontSize: 15, fontWeight: 800, color: "#FFFFFF", marginBottom: 2 }}>Compliance & Consent Dashboard</div>
          <div style={{ fontSize: 11, color: "rgba(255,255,255,0.55)" }}>Client Deployer View — Bosch Automotive CH · Recruiter Admin</div>
        </div>
        <div style={{ textAlign: "right" }}>
          <div style={{ fontSize: 10, color: "rgba(255,255,255,0.5)", marginBottom: 2 }}>Jurisdiction</div>
          <div style={{ display: "flex", gap: 4 }}>
            {["GDPR", "EU AI Act", "nLPD/nFADP"].map(j => (
              <span key={j} style={{ fontSize: 9, fontWeight: 700, color: "#FFFFFF", background: "rgba(255,255,255,0.12)", padding: "2px 7px", borderRadius: 4 }}>{j}</span>
            ))}
          </div>
        </div>
      </div>
      ...
      <div style={{ display: "flex", gap: 0, marginBottom: 16, background: "#F3F4F6", borderRadius: 10, padding: 4 }}>
        <button onClick={() => setTab("consent")} style={{
          flex: 1, padding: "10px 16px", fontSize: 13, fontWeight: 700, border: "none", cursor: "pointer", borderRadius: 8, transition: "all 0.2s",
          background: tab === "consent" ? "#FFFFFF" : "transparent",
          color: tab === "consent" ? "#0F2A4A" : "#6B7280",
          boxShadow: tab === "consent" ? "0 1px 4px rgba(0,0,0,0.1)" : "none",
        }}>
          🔐 Consent Management
          <div style={{ fontSize: 10, fontWeight: 400, opacity: 0.7 }}>GDPR · R-02, R-03, R-04, R-27, R-28</div>
        </button>
        <button onClick={() => setTab("aiact")} style={{
          flex: 1, padding: "10px 16px", fontSize: 13, fontWeight: 700, border: "none", cursor: "pointer", borderRadius: 8, transition: "all 0.2s",
          background: tab === "aiact" ? "#FFFFFF" : "transparent",
          color: tab === "aiact" ? "#0F2A4A" : "#6B7280",
          boxShadow: tab === "aiact" ? "0 1px 4px rgba(0,0,0,0.1)" : "none",
        }}>
          🤖 AI Act Deployer Obligations
          <div style={{ fontSize: 10, fontWeight: 400, opacity: 0.7 }}>Art. 4, 12, 13, 14, 26(7), 86 · Deadline Aug 2026</div>
        </button>
      </div>

      {tab === "consent" ? <ConsentTab /> : <AIActTab />}
    </div>
  );
}
